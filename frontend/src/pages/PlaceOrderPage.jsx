...
import CartTotalComponent from '../components/CartTotalComponent.jsx';
import { ShopContext } from '../context/ShopContext.jsx';
import { assets } from '../assets/img/assets.js';

export default function PlaceOrderPage() {
   const [method, setMethod] = useState('cod');
   const { navigate, cartItems, setCartItems, getCartAmount, deliveryFee, products } = useContext(ShopContext);
   ...
   return (
      <form onSubmit={handleSubmit}
         ...
         <div className='flex flex-col gap-4 w-full sm:max-w-[480px]'>
            ...
            <input required onChange={onChangeHandler} name='email'
                   value={formData.email}
                   className='border border-gray-300 rounded py-1.5 px-3.5 w-full'
                   type="email" placeholder='Email address'/>
            <input required onChange={onChangeHandler} name='street'
                   value={formData.street}
                   className='border border-gray-300 rounded py-1.5 px-3.5 w-full'
                   type="text" placeholder='Street address'/>
            <div className='flex gap-3'>
               <input required onChange={onChangeHandler} name='city'
                      value={formData.city}
                      className='border border-gray-300 rounded py-1.5 px-3.5 w-full'
            ...
            <input required onChange={onChangeHandler} name='phone'
            ...
            <div className='mt-12'>
               ...
               <div className='flex gap-3 flex-col lg:flex-row'>
                  ...
                  <div onClick={() => setMethod('cod')}
                     ...
                     <p className='text-gray-500 text-sm font-medium mx-4'>CASH ON
                        DELIVERY</p>
                  </div>
               </div>
               ...
               <div className='w-full text-end mt-8'>
                  <button type='submit'
                          className='bg-black text-white px-16 py-3 text-sm'>PLACE
                     ORDER
                  </button>
               </div>
            </div>
